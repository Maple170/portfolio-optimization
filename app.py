...
- 古典ソルバー (scipy): リアルタイム実行
- 量子ソルバー (Amplify): 事前計算済みの結果を表示
...
# 量子ソルバーの事前計算済み結果を読み込む
with open("assets/quantum_result.json") as f:
    quantum_result = json.load(f)

p_q = quantum_result["portfolio"]
r_q = quantum_result["return_rate"]
v_q = quantum_result["variance"]
r_q_eval = np.array(quantum_result["eval"]["values"])
...
with st.sidebar:
    st.header("パラメータ設定")
    st.info("古典ソルバーのパラメータです。量子ソルバーは γ=20、運用日数=20 で事前計算済みです。")
    ...
    run = st.button("古典ソルバーで最適化を実行", type="primary", use_container_width=True)
...
if run:
    ...
    for ax, portfolio, return_rate, variance, title in [
        ...
        (axes[1], p_q, r_q, v_q, "Quantum (Amplify) ※事前計算"),
    ]:
    ...
    table = pd.DataFrame({
        ...
        "Quantum":   [f"{r_q*100:.2f}%", f"{v_q:.6f}", len(p_q), "事前計算済み"],
    ...
    st.info("サイドバーでパラメータを設定して「古典ソルバーで最適化を実行」を押してください。")
